function renderSprites(ctx, zBuffer, mapData) {
  ...
  spriteList.forEach(sprite => {
    ...
    if (sprite.type === 'enemy' || sprite.type === 'dead') {
      ...
      // Alert indicator when hurt
      if (e.hurtTimer > 0.15 && sprite.dist < 10) {
        const alertT = performance.now() * 0.01;
        const alertBob = Math.sin(alertT * 20) * 2;
        ctx.fillStyle = '#ff4444';
        ctx.font = 'bold ' + Math.floor(spriteWidth * 0.4) + 'px monospace';
        ctx.textAlign = 'center';
        ctx.shadowColor = '#ff0000';
        ctx.shadowBlur = 8;
        ctx.fillText('!', drawX + spriteWidth / 2, drawY - spriteHeight * 0.15 + alertBob);
        ctx.shadowBlur = 0;
      }
