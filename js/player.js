...
function checkItemPickups(mapData) {
  ...
  if (tile === 6 || tile === 12) {
    if (player.health < CONFIG.MAX_HEALTH) {
      ...
      mapData[my][mx] = 0;
      playPickup();
      spawnParticles(mx + 0.5, my + 0.5, 8, '#00ff44', 2, 0.5);
      spawnParticles(mx + 0.5, my + 0.5, 5, '#ffffff', 1.5, 0.3);
      showPickupMessage('+' + heal + ' HP', '#00ff44');
    ...
    if (player.ammo.plasma < CONFIG.WEAPONS.plasma.maxAmmo) {
      ...
      added = true; msg += '+20 CELLS';
    }
    if (added) {
      mapData[my][mx] = 0;
      playPickup();
      spawnParticles(mx + 0.5, my + 0.5, 6, '#ffaa00', 2, 0.4);
      showPickupMessage(msg, '#ffaa00');
    }
  } else if (tile === 5) {
    checkLevelComplete();
  }
