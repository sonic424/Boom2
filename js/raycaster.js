...
function loadAnimatedFrames(callback) {
  const files = [
    'images/wall_brick.png',
    'images/wall_metal.png',
    'images/wall_blood.png',
    'images/12ge.png',
    'images/im1212121212e.png'
  ];
  let loaded = 0;
  const T = CONFIG.TEX_SIZE;
  files.forEach((src, i) => {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(img, 0, 0, T, T);
      animFrames[i] = c;
      animFrameData['af'+i] = ctx.getImageData(0, 0, T, T).data;
      loaded++;
      if (loaded === files.length && callback) callback();
    };
    img.onerror = () => { loaded++; if (loaded === files.length && callback) callback(); };
...
function renderFrame(ctx, mapData) {
  ...
  for (let x = 0; x < W; x++) {
    ...
    if (wt === 11) {
      if (animFrameData['af0']) {
        const frame = Math.floor(performance.now() * 0.012) % ANIM_FRAME_COUNT;
        texData = animFrameData['af' + frame];
      } else if (easterEggData['egg0']) {
