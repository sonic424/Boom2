...
function playScare(type) {
  if (!audioCtx) return;
  try {
    const now = audioCtx.currentTime;
    const dur = 0.7;

    // Noise blast (scream breath)
    const noiseBuf = synthNoiseBuf(dur);
    if (noiseBuf) {
    ...
    let freq1 = 200, freq2 = 180, freq3 = 800, freq4 = 500, freq5 = 1000, lfoRate = 25, lfoDepth = 60;

    if (type === 'glitch') {
      freq1 = 400; freq2 = 50; freq3 = 1200; freq4 = 800; freq5 = 200;
      lfoRate = 50; lfoDepth = 120;
    } else if (type === 'blood') {
      freq1 = 150; freq2 = 120; freq3 = 400; freq4 = 200; freq5 = 600;
      lfoRate = 15; lfoDepth = 40;
    }
    ...
    osc1.frequency.linearRampToValueAtTime(freq3, now + 0.15);
    osc1.frequency.linearRampToValueAtTime(freq4, now + 0.35);
    osc1.frequency.linearRampToValueAtTime(freq5, now + 0.5);
    ...
    osc2.frequency.linearRampToValueAtTime(freq3 * 0.9, now + 0.12);
    osc2.frequency.linearRampToValueAtTime(freq4 * 0.8, now + 0.3);
