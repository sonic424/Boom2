...
function shootWeapon() {
  ...
  if (player.weapon === 'pistol') {
    ...
    player.weaponRecoil = 0.12;
    player.weaponKickX = -2 + Math.random() * 4;
    player.weaponKickY = -6;
    shellEjectTimer = 0.4;
    ...
    weaponMuzzleFlash = 0.2;
    player.weaponRecoil = 0.25;
    player.weaponKickX = -4 + Math.random() * 8;
    player.weaponKickY = -12;
    shellEjectTimer = 0.6;
    ...
    weaponMuzzleFlash = 0.1;
    player.weaponRecoil = 0.06;
    player.weaponKickX = -1 + Math.random() * 2;
    player.weaponKickY = -3;
  }
}
...
function fireBullet(damage, spread) {
  ...
  while (!hit && curDist < maxDist) {
    ...
    if (tile > 0 && tile !== 5 && tile !== 6 && tile !== 7 && tile !== 12 && tile !== 13 && tile !== 14 && tile !== 15 && tile !== 16 && tile !== 18 && tile !== 20 && tile !== 21 && tile !== 22 && tile !== 23) { hit = true; break; }
  }
...
function drawWeapon(ctx, dt) {
  ...
  if (recoilFactor > 0) {
    player.weaponKickY += 30 * dt;
    player.weaponKickX *= 0.8;
  } else {
    player.weaponKickY += (0 - (player.weaponKickY || 0)) * Math.min(1, 8 * dt);
    player.weaponKickX = (player.weaponKickX || 0) * Math.pow(0.15, dt);
  }
  ...
  if (isMoving) {
    ...
    player.weaponBobX = Math.sin(t) * 4;
    player.weaponBobY = Math.abs(Math.cos(t)) * 3;
  } else {
  ...
  if (weaponMuzzleFlash > 0) {
    ...
    const flashSize = 35 + Math.abs(Math.sin(performance.now() * 0.05)) * 25;
    const fx = W / 2 + 70;
    ...
    fg.addColorStop(0.2, player.weapon === 'plasma' ? '#44ffaa' : '#ffcc00');
    fg.addColorStop(0.6, player.weapon === 'plasma' ? '#00ff88' : '#ff8800');
    ...
    ctx.beginPath();
    ctx.arc(fx, fy, flashSize * 0.25, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  ctx.drawImage(sprite, wx + kx, wy + ky, sw, sh);
}
