...
function killEnemy(index) {
  ...
  spawnParticles(e.x, e.y, 20, '#aa0000', 4, 0.6);
  spawnParticles(e.x, e.y, 12, '#ff4400', 3, 0.4);
  spawnParticles(e.x, e.y, 8, '#662200', 2, 0.8);
  spawnParticles(e.x, e.y, 5, e.type === 'spectre' ? '#4488cc' : '#884422', 5, 0.3);
  checkAllEnemiesDead();
}
...
function enemyAttack(enemy, index) {
  ...
  if (enemy.type === 'demon' || enemy.type === 'spectre') {
    if (dist < 1.5) { playerTakeDamage(config.damage); triggerCombatScare(0.8); }
